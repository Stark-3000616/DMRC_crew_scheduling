...
def column_generation2(graph, service_dict, init_column_generator = "random", mpc_timeout =30, pricing_method = "topological sort", iterations = None, verbose = False):
...
def column_generation4(graph, service_dict, current_duties, selected_vars, pricing_method = "topological sort", iterations = None, verbose = False):
    ...
    while True:
        ...
        print("LP solve time: ", lp_end_time - lp_start_time)
        # duty, reduced_cost = generate_new_column(graph, service_dict, duals, method = pricing_method, verbose = verbose)
        ...
        if duty in current_duties:
            ...
            if verbose:
                print("Unique Column found!")
                print(f"Column Generated through {pricing_method} in:  {end_time_1 - start_time_1:.6f} seconds")
                print("Generated duty Main:", duty, "Reduced cost Main (shortest path):", reduced_cost)
                duty_dur =0 
                for serv in duty:
                    duty_dur += service_dict[serv].serv_dur
...
def cg_heuristics(graph, service_dict, current_duties, threshold, n = 1, iterations = 1000, verbose = False):
    selected_vars = []
    ...
    while len(selected_vars) < len(service_dict):
        ...
        start_time_1 = time.time()
        current_duties, final_duties, selected_duties, obj, basis = column_generation4(graph, service_dict, selected_vars=selected_vars, current_duties=current_duties, pricing_method = "topological sort", iterations = iterations, verbose = verbose)
